...
def process_excel_to_json(input_file, output_file):
    ...
    for row in sheet.iter_rows(min_row=2, max_col=2, values_only=True):
        ...
        conversation = {
            "system": system_value,
            "input": '货币代码'+str(row[0])+'是什么',
            "output": '根据贸易便利化术语，'+'货币代码'+str(row[0])+'是'+str(row[1])
        }
        conversation1 = {
            "system": system_value,
            "input": str(row[1])+'的货币代码是什么',
            "output": '根据贸易便利化术语，'+str(row[1])+'的货币代码是'+str(row[0])
        }
...
process_excel_to_json('货币.xlsx', 'trade20240124-train.jsonl')

...
def process_excel_to_json(input_file, output_file):
    ...
    for row in sheet.iter_rows(min_row=2, max_col=3, values_only=True):
        ...
        conversation = {
            "system": system_value,
            "input": '关区代码'+str(row[0])+'是什么',
            "output": '根据贸易便利化术语，'+'关区代码'+str(row[0])+'是'+row[2]
        }
        conversation1 = {
            "system": system_value,
            "input": row[2]+'的关区代码是什么',
            "output": '根据贸易便利化术语，'+row[2]+'的关区代码是'+str(row[0])
        }
...
process_excel_to_json('关区代码.xlsx', 'trade20240124-train.jsonl')
